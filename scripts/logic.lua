...
function can_adventure()
    if difficulty_is_easy() > 0 then --easy
        local value = weapons_iron() > 0 and (crafting_ingots() > 0 or campfire() > 0) and has_iron() > 0 and
        (deathlink() == 0 or bed() > 0)
        if ENABLE_DEBUG_LOG then
    ...
    elseif difficulty_is_hard() > 0 then --hard
        ...
        local value = weapons_cobble() > 0 and (crafting_ingots() > 0 or campfire() > 0) and
        (deathlink() == 0 or bed() > 0)
        if ENABLE_DEBUG_LOG then
            print(string.format("can_adventure: value: %s", value))
        end
